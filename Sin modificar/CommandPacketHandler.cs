...
namespace OpenNos.Handler
{
    public class CommandPacketHandler : IPacketHandler
    {
        ...
        public void Act4Connect(Act4ConnectPacket act4ConnectPacket)
        {
            ...
            {
                case 0:
                    ServerManager.Instance.ChangeMap(player.Character.CharacterId, 1, 108, 115);
        ...
        public void Command(HelpPacket helpPacket)
        {
            ...
            {
                Session.SendPacket(Session.Character.GenerateSay("$SetPoint <name>", 12));
                Session.SendPacket(Session.Character.GenerateSay("$Point <name>", 12));
                Session.SendPacket(Session.Character.GenerateSay("$$UnsetPoint <name>", 12));
                Session.SendPacket(Session.Character.GenerateSay("$ListPoint", 12));
        ...
        public void HelpMe(HelpMePacket packet)
        {
            ...
            foreach (ClientSession team in ServerManager.Instance.Sessions.Where(s => s.Account.Authority >= AuthorityType.Moderator))
            {
                ...
                team.SendPacket(team.Character.GenerateSay($"Player {Session.Character.Name} need help!",
                    12));
                team.SendPacket(team.Character.GenerateSay($"Reason: {packet.Message}", 12));
                team.SendPacket(team.Character.GenerateSay("Wait for an answer!", 12));
                team.SendPacket(Session.Character.GenerateSpk("Let's start talking here", 5));
